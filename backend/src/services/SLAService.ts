...
export class SLAService {
  ...
  /**
   * Get AI-enhanced SLA status for a ticket
   */
  public async getAIEnhancedSLAStatus(ticket: TicketEntity, currentTime?: Date): Promise<SLAStatus & {
    aiPrediction?: {
    ...
    const baseSLAStatus = this.getSLAStatus(ticket, currentTime);
    
    try {
      // Get AI prediction for enhanced risk assessment
      const aiPrediction = await aiService.predictSLA({
        ticket_id: ticket.id,
        current_time: currentTime || new Date(),
        priority: ticket.priority,
        ...
        progress_percentage: baseSLAStatus.progressPercentage
      });

      if (aiPrediction.success && aiPrediction.result) {
        // Combine traditional risk score with AI prediction
        const combinedRiskScore = Math.max(
          baseSLAStatus.riskScore,
          aiPrediction.result.breach_probability
        );
        ...
        return {
          ...
          aiPrediction: aiPrediction.result
        };
      }
    } catch (error) {
      logger.warn('AI SLA prediction failed, using traditional calculation', {
        ticketId: ticket.id,
        error: (error as Error).message
      });
    }

    return baseSLAStatus;
  }

  /**
   * Batch AI-enhanced SLA monitoring for multiple tickets
   */
  public async batchAIEnhancedSLACheck(
    ...
    // Process tickets in batches to avoid overwhelming AI service
    const batchSize = 10;
    for (let i = 0; i < tickets.length; i += batchSize) {
      const batch = tickets.slice(i, i + batchSize);
      
      const batchPromises = batch.map(async (ticket) => {
        const enhancedSLA = await this.getAIEnhancedSLAStatus(ticket);
        
        return {
          ticket,
          slaStatus: enhancedSLA,
          aiPrediction: enhancedSLA.aiPrediction,
          needsAttention: enhancedSLA.riskScore >= riskThreshold || enhancedSLA.isOverdue
        };
      });
    ...
    logger.info('Batch AI-enhanced SLA check completed', {
  ...
  /**
   * Get AI-powered SLA insights for dashboard
   */
  public async getAISLAInsights(): Promise<{
    totalAtRisk: number;
    criticalRisk: number;
    ...
    try {
      // Get active tickets
      const searchResult = await this.ticketRepository.search({
        filters: {
      ...
      const enhancedResults = await this.batchAIEnhancedSLACheck(searchResult.tickets, 0.5);
      
      // Analyze results
      const atRiskTickets = enhancedResults.filter(r => r.slaStatus.riskScore >= 0.7);
      const criticalRiskTickets = enhancedResults.filter(r => r.slaStatus.riskScore >= 0.85);
      
      const totalRiskScore = enhancedResults.reduce((sum, r) => sum + r.slaStatus.riskScore, 0);
      const averageRiskScore = totalRiskScore / enhancedResults.length;

      // Collect risk factors from AI predictions
      const riskFactors = new Map<string, number>();
      const recommendations = new Set<string>();

      enhancedResults.forEach(result => {
        if (result.aiPrediction?.risk_factors) {
          result.aiPrediction.risk_factors.forEach((factor: string) => {
            riskFactors.set(factor, (riskFactors.get(factor) || 0) + 1);
          });
        }
        
        if (result.aiPrediction?.recommendations) {
          result.aiPrediction.recommendations.forEach((rec: string) => {
      ...
      return {
        ...
        criticalRisk: criticalRiskTickets.length,
      ...
      logger.error('Failed to get AI SLA insights', { error: (error as Error).message });
      throw error;
    }
  }
}